...
Usage:
    $ python L2_processing.py year drone period [downsample]
where
...
def main():
    ...
    ds_mean = wind_dir_twice(ds_mean, 'UWND', 'VWND')
    #
    print('Transforming to streamwise flow...')
    ds_2D = rotation_xy(ds_2D, 'UWND', 'VWND', np.radians(ds_mean['arctan_VoverU'].data), 'math')
    #
    print('Detrending...')
    ...
    ds_tau['wind_dir'] = ds_mean['wind_dir']
...
def add_tau_metadata(ds, pd, freq):
    ...
    if 'taus' in ds:
        ...
        ds_out['taus'].attrs['long_name'] = \
            'Downwind (or streamwise or alongstream) wind stress at ocean surface'
        ds_out['taus'].attrs['sign_convention'] = \
            'A positive downward downwind stress is a downward flux of momentum in the direction of the mean wind, which accelerates the ocean in the direction of the mean wind and deccelerates the atmosphere.'
    ...
    if 'tauc' in ds:
        ...
        ds_out['tauc'].attrs['long_name'] = \
            'Crosswind wind stress at ocean surface'
        ds_out['tauc'].attrs['sign_convention'] = \
            'A positive downward crosswind stress is a downward flux of momentum at a right angle to the mean wind, which accelerates the ocean in the direction 90 degrees anticlockwise of the mean wind.'
