...
Usage:
    $ python wind_speed_drag_coeff_scatter_2panel.py
...
sys.path.append( os.path.expanduser('~') +
                 '/Documents/code/saildrone/SaildroneCovarianceFlux/src/')
import config
...
def main():
    ...
    # Load data.
    ds_list_mag = get_ds_list('mag', years, drones)
    ds_list_stream = get_ds_list('stream', years, drones)
    
    # Plot scatter.
    plot_drag_scatter(ds_list_mag, ds_list_stream,
                      years, drones,
                      row_list, col_list)
    # 
    return
...
def get_ds_list(cd_type, years, drones):
...
def plot_drag_scatter(ds_l_m, ds_l_s, yrs, ids, rows, cols):
    
    # Set up figure.
    fig, axs_arr = plt.subplots(2, 1,
                                sharex=True, sharey=False,
                                figsize=(5,8))
    
    for ids, ds_l in enumerate([ds_l_m, ds_l_s]):
        ...
        if ids == 0:
            ...
            sys.exit('Too many datasets passed to plot loop.')
        ...
        if ids == 0:
            ...
            axs.text(0.95, 0.05, "a", transform=axs.transAxes,
                     horizontalalignment='center',
                     verticalalignment='center',
                     bbox=dict(edgecolor='black', facecolor='none',
                               boxstyle="round"))
        elif ids == 1:
            axs.text(0.95, 0.05, "b", transform=axs.transAxes,
                     horizontalalignment='center',
                     verticalalignment='center',
                     bbox=dict(edgecolor='black', facecolor='none',
                               boxstyle="round"))
    ...
    plot_filename = config.plot_dir + \
        'saildrone/' + \
        'wind_speed_drag_coeff_scatter_2panel.'
